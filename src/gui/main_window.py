...
class MainWindow:
    ...
    def load_all_configs(self):
        ...
        try:
            ...
            self.target_ra.set(coords.get("ra", 0.0))
            self.target_dec.set(coords.get("dec", 0.0))
            self.target_ra_str.set(coords.get("ra_str", ""))
            self.target_dec_str.set(coords.get("dec_str", ""))
            mags = target_config.get("magnitudes", {})
            self.target_mag_b.set(mags.get("B", 0.0))
            self.target_mag_v.set(mags.get("V", 0.0))
            self.target_mag_r.set(mags.get("R", 0.0))
            self.target_mag_i.set(mags.get("I", 0.0))
